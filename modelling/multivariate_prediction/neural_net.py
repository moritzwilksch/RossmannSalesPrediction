...
def objective(trial):
    ...
    model.fit(x=train_in, y=ytrain_scaled, validation_data=(val_in, yval_scaled), epochs=10, batch_size=32, callbacks=[mcp])
...
flats = [keras.layers.Flatten()(x) for x in emb_layers + [num_input]]
concat = keras.layers.Concatenate()(flats)
hidden_dense = keras.layers.Dense(units=640, activation='tanh', kernel_regularizer=keras.regularizers.l2())(concat)
hidden_dense = keras.layers.Dense(units=3200, activation='tanh', kernel_regularizer=keras.regularizers.l2())(hidden_dense)
out = keras.layers.Dense(units=1115, activation='linear')(hidden_dense)
